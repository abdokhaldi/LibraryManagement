...
using DVLD_Project_v2.DrivingLicenseServicesScreens.LocalDrivingLicenseApplications;
using System;
...
namespace DVLD_project.Applications
{
    public partial class frmManageLDLApplications : Form
    {
        ...
        private void RefreshLDLApplicationsList()
          {
            _dataGridLDVApplications.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
             _dt = clsLDLApplications.GetLDLApplicationsList();
            _dataGridLDVApplications.DataSource = _dt;
            _lblRecords.Text = _dt.Rows.Count.ToString();
        }
        private void frmManageLDLApplication_Load(object sender, EventArgs e)
        {
            txtSearchApplication.Visible = false;
            cbFilterStatus.Visible = false;
           // dataGridLDVApplications.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.AllCells;
            RefreshLDLApplicationsList();
            

            for (int i=0;i<dataGridLDVApplications.ColumnCount;i++)
            {
                dataGridLDVApplications.Columns[i].Width = 150;
            }
            dataGridLDVApplications.Columns[3].Width = 250;
            dataGridLDVApplications.Columns[5].Width = 200;
        ...
        private void btnAddApplication_Click(object sender, EventArgs e)
        {
            ...
            newLocalLicenseApplication.OnRefreshLDLApplicationList += RefreshLDLApplicationsList;
       ...
       private void _ContextMenuItemsEnabling(bool[] bools)
        ...
        void toolStripMenuItemCheckedBools(short i=-1)
        {
            if (i != -1)
            {
                _ContextMenuItemsEnabling(new bool[] { true, false, false, false, false, true, false, true });
            }
            else
            {
                _ContextMenuItemsEnabling(new bool[] { true, true, true, true, true, false, true, true });
            }
        }
      ...
      private void scheduleStreetTestToolStripMenuItem_Click(object sender, EventArgs e)
        ...
        private void txtSearchApplication_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void cbFilterStatus_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cbFilterStatus.Text == "None")
            {
                RefreshLDLApplicationsList();
                return;
            }
            dv.RowFilter = $"Status='{cbFilterStatus.Text.Trim()}'";
        }

        private void showApplicationDetaisToolStripMenuItem_Click(object sender, EventArgs e)
        {
            frmDLApplicationInfo ldlApplicationCard = new frmDLApplicationInfo((int)_CurrentCell(enCurrentCell.LDLAppID));
            ldlApplicationCard.Show();
        }
        ...
        private void lblManageApp_LOGO_Click(object sender, EventArgs e)
        {
